...
const bookingFormSchema = z.object({
  ...
  time: z.string({ message: "A time is required." }),
...
export function BookingForm({ serviceId, providerId, service }: BookingFormProps) {
  ...
  const mutation = useMutation({
    mutationFn: (data: Record<string, unknown>) => apiPost("/api/bookings", data),
    onSuccess: () => {
      toast({ title: "Booking Request Sent", description: "The provider has been notified of your request." });
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <FormField control={form.control} name="notes" render={({ field }) => (
          <FormItem>
            ...
            <FormControl><Textarea placeholder="Describe your requirements..." className="resize-none" {...field} /></FormControl>
            <FormMessage />
          </FormItem>
        )} />
