...
import { Star, MessageCircle, CheckCircle, Briefcase, Calendar } from "lucide-react";
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          <div className="flex flex-col items-center p-2 bg-muted rounded-lg">
            <span className="font-bold text-lg">{provider.responseTime ? `${provider.responseTime}h` : "-"}</span>
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <span>{provider.providerType === "licensed_professional" ? "Professional" : "Freelancer"}</span>
          </div>
          <div className="flex items-center gap-2 text-muted-foreground">
            <Calendar className="h-4 w-4" />
            <span>Member since {new Date(provider.createdAt).toLocaleDateString("en-US", { month: "short", year: "numeric" })}</span>
          </div>
        </div>
