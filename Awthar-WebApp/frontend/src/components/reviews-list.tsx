...
export function ReviewsList({ providerId, reviews: initialReviews }: ReviewsListProps) {
  const { data: fetchedReviews, isLoading } = useQuery<ReviewWithCustomer[]>({
    queryKey: [`/api/reviews/provider/${providerId}`],
    queryFn: getQueryFn({ on401: "returnNull" }),
    enabled: !initialReviews,
  });
          ...
          <CardContent className="p-4">
            <div className="flex justify-between items-start">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-sm">{review.customer.firstName} {review.customer.lastName}</p>
              ...
              <div className="flex">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className={cn("h-4 w-4", star <= review.rating ? "fill-yellow-400 text-yellow-400" : "text-muted-foreground/30")} />
                ))}
              </div>
            </div>
            <div className="mt-3 text-sm leading-relaxed"><p>{review.comment}</p></div>
            {review.response && (
