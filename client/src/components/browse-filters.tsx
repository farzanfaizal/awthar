        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">
              {selectedCount > 0 ? `${selectedCount} Categories` : "Category"}
            </span>
          </div>
      ...
      <PopoverTrigger asChild>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">
              {hasCustomRange ? `${priceRange[0]}-${priceRange[1]} AED` : "Price"}
            </span>
          </div>
          <ChevronDown className="h-4 w-4 text-muted-foreground ml-2" />
        </Button>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            <Slider
              ...
              step={50}
              value={priceRange}
              onValueChange={(vals) => onPriceChange(vals as [number, number])}
            />
            <div className="flex items-center justify-between">
              <div className="bg-muted px-3 py-1.5 rounded-lg">
                <span className="text-sm font-medium">{priceRange[0]} AED</span>
              </div>
              <span className="text-sm text-muted-foreground">to</span>
              <div className="bg-muted px-3 py-1.5 rounded-lg">
                <span className="text-sm font-medium">{priceRange[1]} AED</span>
              </div>
            </div>
            <p className="text-xs text-muted-foreground">
              Services priced between {priceRange[0]} and {priceRange[1]} AED
            </p>
          </div>
        </div>
      </PopoverContent>
              ...
              <div className="space-y-2">
                <button
                  ...
                  className={cn(
                    ...
                    !minRating && "border-primary bg-primary/5"
                  )}
                >
                  Any rating
                </button>
                {[4, 4.5].map((rating) => (
                  <button
                    ...
                    className={cn(
                      "w-full text-left px-3 py-2 rounded-lg border text-sm transition-colors",
                      minRating === rating && "border-primary bg-primary/5"
                    )}
                  >
                    {rating}+ stars
                  </button>
